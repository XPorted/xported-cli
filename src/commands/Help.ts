...
const Help = new Command({
	...
	action: async (option, parameters, rawArguments) => {
		const CommandRegistry = (await import('../utils/CommandRegistry.js'))
			.default;

		const commandParameter = parameters.find(
			param => param.name === 'command'
		) as ValuedParameter | undefined;
		...
		if (!commandParameter) {
			...
			terminal
				.bold('Use "')
				.cyan('xported-cli help --command <command>')
				.bold('" for more information on a specific command.\n');
		} else {
			...
			terminal.bold
				.cyan(`${commandInfo.name}`)
				.gray(` - ${commandInfo.description}.\n`);
			if (commandInfo.methods === '*') commandInfo.methods = [];
			...
			if (commandInfo.parameters.length > 0) {
				...
				for (const parameter of commandInfo.parameters) {
					if (parameter.type === 'valuedParameter') {
						terminal
							.cyan(`\t--${parameter.name} <${parameter.name}>`)
							.gray(` - ${parameter.description}.\n`);
					} else {
						terminal
							.cyan(`\t--${parameter.name}`)
							.gray(` - ${parameter.description}.\n`);
			...
			terminal.bold('Usage:\n');
			terminal.cyan(
				`\txported-cli ${commandInfo.name}${commandInfo.methods.length > 0 ? ' <method>' : ''}${commandInfo.parameters.length > 0 ? ' [parameters]' : ''}\n`
			);

			terminal('\n');
			terminal
				.bold('Use "')
				.cyan('xported-cli help')
				.bold('" to see all commands.\n');
		}
	}
});
